    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
      ...
      <header className="border-b border-slate-700 bg-slate-900/50 backdrop-blur-md sticky top-0 z-30">
        <div className="max-w-7xl mx-auto px-6 py-5">
          ...
          <div>
            <h1 className="text-3xl font-bold text-white">Search by Year</h1>
            <p className="text-amber-400 text-lg font-semibold">{year}</p>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-6 py-12">
        ...
        <div className="mb-8 bg-slate-800/50 border border-slate-700 rounded-lg p-4">
          <p className="text-slate-300 font-medium">
            Found <span className="text-amber-400 font-bold">{filteredMovies.length}</span> {filteredMovies.length === 1 ? 'film' : 'films'} from {year}
          </p>
        </div>
        ...
        {filteredMovies.length === 0 ? (
          <div className="text-center py-24 bg-slate-800/30 border border-slate-700 rounded-2xl">
            <p className="text-slate-400 text-lg font-medium">No films found from {year}</p>
          </div>
        ) : (
