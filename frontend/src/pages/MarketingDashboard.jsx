import { LogOut, ArrowLeft, TrendingUp, ChevronDown } from 'lucide-react';
...
const MarketingDashboard = ({ movies = [] }) => {
  ...
  const genres = ['All Genres', ...new Set(movies.flatMap(movie => movie.genres || []))].sort();
            ...
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="relative w-56">
                <button
                  ...
                  className="w-full px-4 py-2 bg-white text-slate-900 rounded-lg text-sm font-medium flex items-center justify-between hover:bg-slate-100 transition-colors"
                >
                  <span>{selectedGenreFilter}</span>
                  <ChevronDown className={`w-4 h-4 transition-transform ${isGenreDropdownOpen ? 'rotate-180' : ''}`} />
                </button>
                {isGenreDropdownOpen && (
                  <div className="absolute top-full left-0 right-0 mt-2 bg-slate-800 border border-slate-600 rounded-lg shadow-lg z-20 max-h-48 overflow-y-auto">
                    {genres.map((genre) => (
                      <button
                        key={genre}
                        onClick={() => {
                        ...
                        className={`w-full px-4 py-2.5 text-left text-sm transition-colors border-b border-slate-700 last:border-b-0 ${
